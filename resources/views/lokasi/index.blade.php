...
<div class="mt-16 mx-10">
  <div class="flex justify-between items-start">
    <h1 class="text-black font-bold text-3xl mb-5">Data Lokasi</h1>
    <div class="flex gap-3 items-center">
      <div class="w-10 h-10 bg-[#5479f7] drop-shadow-[0_3px_2px_rgba(0,0,0,0.4)] ease-in-out transition duration-150 text-center rounded-full">
        <a href="{{ route('lokasi.create') }}" class="text-4xl no-underline text-white drop-shadow-[0_3px_2px_rgba(0,0,0,0.4)]">+</a>
      </div>
    ...
    <table class="w-full text-sm text-left text-blue-100">
      <thead  class="text-xs text-white text-center uppercase bg-[#5479f7]">
        <tr>
          ...
          <th scope="col"  class="px-3 py-3">Kode Lokasi</th>
          <th scope="col"  class="px-3 py-3">Nama Gedung</th>
          <th scope="col"  class="px-3 py-3">Nama Lantai</th>
      ...
      <tbody class="text-black text-center">
        ...
        @foreach ($lokasi as $alat => $row)
          <tr class="{{ ($alat+1)%2 == 0 ? 'bg-white border-b-2 border-gray-300' : '' }}">
            <th scope="row" class="px-3 py-2 font-medium whitespace-nowrap">{{$alat + $lokasi -> firstItem()}}</th>
            <td class="px-3 py-2">{{$row -> kode_lokasi}}</td>
            <td class="px-3 py-2">{{$row -> nama_gedung}}</td>
            <td class="px-3 py-2">{{$row -> lantai}}</td>
            <td class="px-3 py-2">
              <div class="rounded-md group py-1 hover:bg-[#f5f5f5] cursor-pointer w-10">
                <span class="material-icons">more_vert</span>
                <form class="shadow-md p-1 hidden z-[9000] group-hover:block mt-1 rounded-md absolute bg-white" action="{{ route('lokasi.destroy',$row->kode_lokasi) }}" method="POST">
    
                    <a href="{{ route('lokasi.edit',$row->kode_lokasi) }}"><i class='bx bx-edit text-2xl text-black hover:bg-[#eaeaea] p-1 rounded'></i></a>
