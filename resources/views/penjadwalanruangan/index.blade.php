        ...
        <div class="flex justify-between items-start">
            ...
            <div class="flex gap-3 items-center">
              <div class="w-10 h-10 bg-[#1e6261] hover:bg-[#184D4C] ease-in-out transition duration-150 text-center rounded-full">
                <a href="{{ route('penjadwalanruangan.create') }}" class="text-4xl no-underline text-white">+</a>
              </div>
        ...
        <div>
            <table class="w-full leading-10 text-sm text-white">
                <tr class="text-center border-b-2 border-white">
                    <th scope="col">No</th>
                    <th scope="col">Id Penjadwalan</th>
                ...
                <tr class="text-center">
                    <th scope="row">{{ $index + $penjadwalanruangan->firstItem() }}</th>
                        <td scope="row">{{ $row->id_penjadwalan }}</td>
                        ...
                        <td scope="row">{{ $row->tanggalMasuk }}</td>
                        <td scope="row">{{ $row->tanggalKeluar }}</td>
                        <td scope="row">{{ $row->statusRuangan }}</td>
                    <td id="hapus-edit" class="text-white">
                        <form action="{{ route('penjadwalanruangan.destroy',$row->id_penjadwalan) }}" method="POST">
           
                            {{-- <a class="btn btn-info" href="{{ route('penjadwalanruangan.show',$penjadwalanruangan->id_penjadwalan) }}">Show</a> --}}
            
                            <a href="{{ route('penjadwalanruangan.edit',$row->id_penjadwalan) }}"><i class='bx bx-edit text-2xl text-white hover:bg-slate-700 bg-slate-600 p-1 rounded'></i></a>
