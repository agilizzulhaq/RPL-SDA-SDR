...
<div class="mt-16 mx-10">
  ...
  <div class="w-full bg-white rounded-xl text-sm text-black-md p-3 border-2 border-green-200">
    <form action="{{ route('prodiregina.update',$prodiregina->id) }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      <div class="mb-3 flex justify-between">
        <label for="namaprodi" class="form-label text-black">Nama Prodi</label>
        <input type="text" name="namaprodi" class="rounded text-sm py-1 px-2 text-black w-[700px]  border-1 border-black" id="namaprodi" value="{{ $prodiregina -> namaprodi }}">
      </div>
