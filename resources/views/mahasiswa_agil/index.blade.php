            ...
            <tr class="{{ ($mahasiswa+1)%2 == 0 ? 'bg-white border-b-2 border-gray-300' : '' }}">
              ...
              <td class="px-3 py-2">
                <div class="rounded-md group py-1 hover:bg-[#f5f5f5] cursor-pointer w-10">
                  ...
                  <form class="shadow-md p-1 hidden z-[9000] group-hover:block mt-1 rounded-md absolute bg-white" action="/mahasiswa-agil/delete/{{ $row -> IDMahasiswa }}" method="POST">
                  <a href="/mahasiswa-agil/edit/{{ $row -> IDMahasiswa }}"><i class='bx bx-edit text-2xl text-black hover:bg-[#eaeaea] p-1 rounded'></i></a>
                  @csrf
                  @method('DELETE')
