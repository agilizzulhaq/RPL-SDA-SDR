...
<div class="mt-16 mx-10">
  <div class="flex justify-between items-start">
    <h1 class="text-black font-bold text-3xl mb-5">Data User</h1>
    <div class="flex gap-3 items-center">
      <div class="w-10 h-10 bg-[#5479f7] drop-shadow-[0_3px_2px_rgba(0,0,0,0.4)] ease-in-out transition duration-150 text-center rounded-full">
        <a href="/addusers" class="text-4xl no-underline text-white drop-shadow-[0_3px_2px_rgba(0,0,0,0.4)]">+</a>
      </div>
      ...
      <form action="/data-master/users" class="h-10 w-20 min-w-[200px]">
    ...
    <table class="w-full text-sm text-left text-blue-100">
      <thead class="text-xs text-white text-center uppercase bg-[#5479f7]">
        <tr>
          ...
          <th scope="col" class="px-3 py-3">ID User</th>
          <th scope="col" class="px-3 py-3">Nama User</th>
          <th scope="col" class="px-3 py-3">Tanggal Lahir</th>
          <th scope="col" class="px-3 py-3">Alamat User</th>
          <th scope="col" class="px-3 py-3">Email User</th>
          <th scope="col" class="px-3 py-3">Role User</th>
          <th scope="col" class="px-3 py-3"></th>
        </tr>
      </thead>
      <tbody class="text-black text-center">
          @foreach ($data as $pengguna => $row)
          <tr class="{{ ($pengguna+1)%2 == 0 ? 'bg-white border-b-2 border-gray-300' : '' }}">
              <th class="px-3 py-2 font-medium whitespace-nowrap" scope="row">{{$pengguna + $data -> firstItem()}}</th>
              <td class="px-3 py-2">{{$row -> id_user}}</td>
              <td class="px-3 py-2">{{$row -> nama_user}}</td>
              <td class="px-3 py-2">{{$row -> tanggal_lahir}}</td>
              <td class="px-3 py-2">{{$row -> alamat_user}}</td>
              <td class="px-3 py-2">{{$row -> email_user}}</td>
              <td class="px-3 py-2">{{$row -> role_user}}</td>
              <td class="px-3 py-2">
                <div class="rounded-md group py-1 hover:bg-[#f5f5f5] cursor-pointer w-10">
                  ...
                  <div class="shadow-md p-1 hidden z-[9000] group-hover:block mt-1 rounded-md absolute bg-white">
                    <a href="/editusers/{{$row -> id_user}}"><i class='bx bx-edit text-2xl text-black hover:bg-[#eaeaea] p-1 rounded'></i></a>
                    <a href="/deleteusers/{{$row -> id_user}}" data-id="{{$row -> id_user}}" data-nama="{{$row -> nama_user}}"><i class='bx bx-trash text-2xl text-black hover:bg-[#eaeaea] p-1 rounded' ></i></a>
                  </div>
                </div>
              </td>
            </tr>
